...
class Section extends ChangeNotifier {
  ...
  Future<void> save(int pos) async {
    ...
    for (final item in items) {
      if (item.imagem is File) {
        final StorageUploadTask task =
            storageRef.child(Uuid().v1()).putFile(item.imagem as File);
    ...
    for (final original in originalItems) {
      if (!items.contains(original)) {
        try {
          final ref =
              await storage.getReferenceFromUrl(original.imagem as String);
          await ref.delete();
  ...
  Future<void> delete() async {
    ...
    for (final item in items) {
      try {
        final ref = await storage.getReferenceFromUrl(item.imagem as String);
        await ref.delete();
        // ignore: empty_catches
      } catch (e) {}
    }
